...
description: Provides comprehensive Tailwind CSS utility-first styling patterns including responsive design, layout utilities, flexbox, grid, spacing, typography, colors, and modern CSS best practices. Use when styling React/Vue/Svelte components, building responsive layouts, implementing design systems, or optimizing CSS workflow. In FantasyIA, `frontend-design` and `project-plaintext-context` override generic examples and theme assumptions in this skill.
...
- read `../frontend-design/SKILL.md` and `../project-plaintext-context/SKILL.md` first;
- do not implement light mode or theme toggles;
- do not treat `bg-white`, `text-gray-*`, or light-card examples below as project defaults;
- prefer project tokens, dark surfaces, and the official FantasyIA palette;
- if a generic example conflicts with the project, the project context wins.
...
<!-- Card component -->
<div class="rounded-3xl border border-white/10 bg-brand-surface-low p-6 text-brand-text">
  <h3 class="text-xl font-bold text-brand-text">Title</h3>
...
function ProductCard({ product }: { product: Product }) {
  return (
    <div className="overflow-hidden rounded-3xl border border-white/10 bg-brand-surface-low sm:flex">
      ...
      <div className="p-6">
        ...
        <button className="mt-4 rounded-full bg-[var(--brand-500)] px-4 py-2 text-black">
          Add to Cart
        </button>
      </div>
    </div>
...
<div class="bg-brand-bg text-brand-text">
  <h1 class="text-brand-text">Title</h1>
</div>
```

FantasyIA note:
- the project is dark-only;
- do not add a dark-mode toggle here.
