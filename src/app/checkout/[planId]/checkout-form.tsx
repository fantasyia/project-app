...
import { mockSubscribe } from "@/lib/actions/checkout";
...
const methodLabels: Record<CheckoutMethod, { label: string; description: string; icon: typeof CreditCard }> = {
  card: {
    label: "Cartao",
    description: "Cartao de credito para liberar o acesso premium.",
    icon: CreditCard,
  },
  pix: {
    label: "Pix",
    description: "Confirmacao rapida para liberar o acesso premium.",
    icon: QrCode,
  },
  wallet: {
    label: "Saldo",
    description: "Opcao reservada para creditos e saldo da plataforma.",
    icon: Wallet,
  },
};
...
export function CheckoutForm({
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await mockSubscribe(creatorId, planId);
      ...
      setError(result?.error || "Nao foi possivel concluir a assinatura agora.");
    });
  }
  ...
  if (!isAuthenticated) {
    return (
      <div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-6">
        ...
        <h3 className="mt-5 text-2xl font-light tracking-tight">Entre para continuar</h3>
        <p className="mt-3 text-sm leading-6 text-brand-text-muted">
          Para proteger sua conta e liberar o acesso corretamente, entre antes de continuar.
        </p>
  ...
  if (isOwner) {
    return (
      <div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-6">
        ...
        <h3 className="mt-5 text-2xl font-light tracking-tight">Plano do proprio creator</h3>
        <p className="mt-3 text-sm leading-6 text-brand-text-muted">
          O sistema bloqueia autoassinatura para manter a regra de negocio coerente.
        </p>
        <Link
          href={`/${creatorHandle}`}
  ...
  if (isSubscribed) {
    return (
      <div className="rounded-[28px] border border-brand-500/20 bg-brand-500/10 p-6">
        ...
        <h3 className="mt-5 text-2xl font-light tracking-tight">Assinatura ja ativa</h3>
        <p className="mt-3 text-sm leading-6 text-brand-text-base">
          Seu acesso a <span className="font-medium text-white">{creatorName}</span> ja esta liberado. Se quiser, volte ao perfil e consuma o conteudo premium agora mesmo.
        </p>
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <Link
            href={`/${creatorHandle}`}
  ...
  if (completed) {
    return (
      <div className="rounded-[28px] border border-brand-500/20 bg-brand-500/10 p-6">
        ...
        <p className="mt-5 text-[10px] uppercase tracking-[0.32em] text-brand-300">Assinatura confirmada</p>
        <h3 className="mt-3 text-3xl font-light tracking-tight">Acesso premium liberado</h3>
        <p className="mt-3 text-sm leading-6 text-brand-text-base">
          O plano <span className="font-medium text-white">{planName}</span> foi confirmado. Seu acesso premium ja esta ativo para o feed e para o perfil de {creatorName}.
        </p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {method === "pix" && (
            <div className="rounded-2xl border border-dashed border-brand-500/20 bg-brand-500/10 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-brand-text-base">
                Confirme para liberar o acesso premium imediatamente.
              </p>
            </div>
          )}
      ...
      <div className="rounded-[28px] border border-white/10 bg-black/35 p-5">
        <div className="flex items-start justify-between gap-3 border-b border-white/8 pb-4">
          <div>
            ...
            <h3 className="mt-2 text-xl font-light tracking-tight">{planName}</h3>
          </div>
        ...
        <div className="mt-4 space-y-3 text-sm text-brand-text-muted">
          <div className="flex items-center justify-between gap-3">
            <span>Ciclo</span>
            <span className="text-white">{planCycleLabel}</span>
          </div>
          <div className="flex items-center justify-between gap-3">
            <span>Creator</span>
          ...
          <div className="flex items-center justify-between gap-3">
            <span>Entrega</span>
            <span className="text-white">Acesso premium imediato</span>
          </div>
        </div>
      </div>
      ...
      >
        <Lock size={14} />
        {pending ? "Processando..." : `Confirmar ${priceLabel} / ${planCycleLabel.toLowerCase()}`}
      </button>

      <p className="text-center text-[10px] uppercase tracking-[0.24em] text-brand-text-muted">
        Assinatura segura; PPV permanece separado e exige unlock proprio
      </p>
    </form>
  );
}
