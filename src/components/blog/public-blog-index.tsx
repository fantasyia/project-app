...
export async function PublicBlogIndex({
  basePath = "/blog",
  backHref = "/",
  embedded = false,
}: {
  ...
  return (
    <div className={embedded ? "bg-black text-white" : "min-h-screen bg-black text-white"}>
      <header className="sticky top-0 z-40 border-b border-white/[0.08] bg-black/90 backdrop-blur-xl">
        <div className={`mx-auto flex w-full ${widthClass} items-center gap-3 px-4 py-3`}>
          <Link
            href={backHref}
      ...
      <main className={`mx-auto w-full ${widthClass} px-4 ${embedded ? "pb-8" : "pb-12"}`}>
        ...
        {featuredArticle ? (
          <section className="grid overflow-hidden rounded-2xl border border-white/8 bg-[#080808] lg:grid-cols-[0.95fr_1.05fr]">
            <Link href={`${basePath}/${featuredArticle.slug}`} className="group relative block min-h-[360px] overflow-hidden">
            ...
            <div className="flex flex-col justify-center p-6 md:p-8">
              ...
              <Link href={`${basePath}/${featuredArticle.slug}`} className="mt-5 text-3xl font-light leading-tight text-white transition hover:text-brand-300 md:text-5xl">
              ...
              <div className="mt-6 flex items-center justify-between border-t border-white/8 pt-4 text-xs text-brand-text-muted">
                ...
                <Link href={`${basePath}/${featuredArticle.slug}`} className="text-brand-400 hover:text-brand-300">
                  Ler artigo
                </Link>
              </div>
            </div>
