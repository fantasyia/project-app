...
export async function getPublicArticleMetadata(slug: string) {
...
export async function PublicArticlePage({
  slug,
  basePath = "/blog",
  embedded = false,
}: {
  ...
  return (
    <div className={embedded ? "bg-black text-white" : "min-h-screen bg-black text-white"}>
      <header className="sticky top-0 z-40 border-b border-white/[0.08] bg-black/90 backdrop-blur-xl">
        <div className={`mx-auto flex w-full ${widthClass} items-center justify-between gap-3 px-4 py-3`}>
          <Link
            href={basePath}
      ...
      <main className={`mx-auto w-full ${widthClass} px-4 ${embedded ? "pb-8" : "pb-14"}`}>
        <article>
          <section className="py-6 md:py-10">
            <div className="flex flex-wrap items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-brand-400">
              <Link href={basePath} className="hover:text-brand-300">Blog</Link>
          ...
          <section className="mt-8 grid gap-8 lg:grid-cols-[220px_minmax(0,1fr)]">
            ...
            <div className="min-w-0">
              <ArticleRichContent content={article.content} blogBasePath={basePath} />
              ...
              <div className="mt-10 rounded-2xl border border-brand-500/20 bg-brand-500/8 p-5">
                ...
                <p className="mt-2 text-sm leading-6 text-brand-text-base">
                  Descubra creators e conecte o conteudo editorial com a experiencia privada premium.
                </p>
                <Link
